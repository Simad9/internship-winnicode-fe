...
function Admin() {
  return (
    <section className='flex flex-col gap-2 p-3 bg-lm-primary rounded-md w-full'>
      <p className='font-ws font-medium text-heading-accs text-[20px]'>Admin</p>
      <div className='flex flex-col gap-3 justify-center items-center md:items-center bg-lm-primary p-4 rounded-md'>
        ...
        <div className='flex flex-col items-start w-full'>
          ...
          <div className='flex flex-row items-center justify-start'>
            ...
            <h3 className='truncate'>{`: User123@example.com`}</h3>
          </div>
        </div>
        <Button to="/admin/edit-account" label="Edit Akun" />
      </div>
    </section>
  )
}
